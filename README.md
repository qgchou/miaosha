...
问题：在同一个用户同时发多个请求的情况下，预减库存有可能会出现商品有剩的情况。  
解决：在判断已经秒杀到的块里加上redis.incr把内存里的库存加1。  
这样子轮询秒杀是否结束就得去数据库访问库存了，因为内存和数据库有一段时间是数据不一致的。  
不对，可以在下单的时候如果判断到库存为0就把这个状态放进内存里，然后轮询秒杀是否结束就可以去内存里查看这个状态，不存在的话就是秒杀为结束。  
## 分布式session  
为什么需要分布式session
